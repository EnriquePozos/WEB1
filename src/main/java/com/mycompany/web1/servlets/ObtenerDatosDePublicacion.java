 ...
 * @author Pozos
 */
@WebServlet(name = "ObtenerDatosDePublicacion", urlPatterns = {"/ObtenerDatosDePublicacion"})
public class ObtenerDatosDePublicacion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ObtenerDatosDePublicacion</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ObtenerDatosDePublicacion at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        conexion con = null;
        Publicacion recetaParaModificar = null;
        ...
        try {
            ...
            recetaParaModificar = publicacionDAO.getPublicacionAModificar(idPublicacion, idUsuarioActual);
        ...
        } else if (recetaParaModificar != null) {
            // Receta encontrada Y pertenece al usuario logueado
            request.setAttribute("PublicacionAModificar", recetaParaModificar); //Para usarlo en formulario en modificarReceta.jsp
            request.getRequestDispatcher("/ModificarReceta.jsp").forward(request, response);
        } else {
            // Receta no encontrada con ese ID o no pertenece al usuario actual
            request.setAttribute("errorGeneral", "La receta que intentas modificar no existe o no tienes permiso para editarla.");
